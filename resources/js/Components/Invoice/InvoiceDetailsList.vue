...
document.body.innerHTML = (`
    <html>
        ...
        <body>
            ...
            <div class="title">${page.props.fromDate?page.props.fromDate:'' } - ${page.props.toDate?page.props.toDate:''}</div>
    ...
    <div v-if="show" class="fixed inset-0 flex justify-center items-center bg-gray-500 bg-opacity-50 z-50">
        <div class="bg-white border rounded-lg shadow-lg p-6 w-[1000px]">
            ...
            <div id="print-invoice" class="max-h-[400px] overflow-y-auto">
                ...
                <div v-for="(item, i) in props.items" :key="i" class="print-page-break">
                    ...
                    <table class="w-full border-collapse table-fixed">
                        <thead v-if="i === 0">
                            ...
                            <th class="border px-2 py-1" style="width: 50px;">No</th> <!-- Reduced width -->
                            <th class="border px-2 py-1">Party's Name</th>
                            <th class="border px-2 py-1" style="width: 300px;">Description</th> <!-- Increased width -->
                        ...
                        <tbody>
                            ...
                            <td v-if="index === 0" :rowspan="item.invoice_products.length" class="border px-2 py-1" style="width: 50px;">
                                {{ i + 1 }}
                            </td>
                            <td v-if="index === 0" :rowspan="item.invoice_products.length" class="border px-2 py-1">
                                {{ item.customer.name }}
                            </td>
                            <td class="border px-2 py-1 overflow-auto"  style="width: 300px; max-width: 300px; word-wrap: break-word; overflow-wrap: break-word; white-space: normal;">{{ product.product.decription }}</td> <!-- Increased width -->
                ...
                <p class="text-right mr-4 font-bold">Total:   {{ page.props.total }} </p>
            </div>
